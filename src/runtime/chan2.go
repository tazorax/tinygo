...
// This implementation of channels supports multiple threads running at the same
// time.
...
//
// - The sender value is stored in the sender 'channelBlockedList', which is
//   really a queue entry. This works for both senders and select operations: a
//   select operation has a separate value to send for each case.
...
//   normal send/receive operations, it starts at chanWaiting and then is
//   changed to chanOperationOk or chanOperationClosed depending on whether the
//   send/receive proceeded normally or because it was closed. For a select
...
const (
	chanWaiting         = iota // waiting for a send/receive operation to continue
	chanOperationOk            // successfully sent or received (not closed)
	chanOperationClosed        // channel was closed, the value has been zeroed
	chanOperationMask   = 0b11
)

type chanQueue struct {
	first *channelBlockedList
}

// Pus the next channel operation to the queue. All appropriate fields must have
// been initialized already.
// This function must be called with the channel lock held.
func (q *chanQueue) push(node *channelBlockedList) {
	node.next = q.first
	q.first = node
}
...
// skipped.
// This function must be called with the channel lock held.
func (q *chanQueue) pop(chanOp uint32) *channelBlockedList {
	for {
		if q.first == nil {
			return nil
		}
		...
		newDataValue := chanOp | uint32(popped.index<<2)
...
func (q *chanQueue) remove(remove *channelBlockedList) {
	n := &q.first
	for *n != nil {
		if *n == remove {
			*n = (*n).next
...
// This function may only be called when the channel is locked and it is known
...
// the send succeeded. The lock must be held when calling this function.
...
func chanSend(ch *channel, value unsafe.Pointer, blockedlist *channelBlockedList) {
	...
	t := task.Current()
	t.SetDataUint32(chanWaiting)
	blockedlist.task = t
	blockedlist.index = 0
	blockedlist.value = value
	ch.senders.push(blockedlist)
	ch.lock.Unlock()
...
// whether the receive operation succeeded. The lock must be held when calling
// this function.
...
func chanRecv(ch *channel, value unsafe.Pointer, blockedlist *channelBlockedList) bool {
	...
	// We can't proceed, so we add ourselfs to the list of receivers and wait
	// until we're awoken.
	t := task.Current()
	t.Ptr = value
	t.SetDataUint32(chanWaiting)
...
// deadlock. This global lock is inefficient, but gets the job done.
//
...
func doChanSelect(recvbuf unsafe.Pointer, states []chanSelectState, ops []channelBlockedList) (uintptr, bool) {
	// Lock everything.
	chanSelectLock.Lock()
	...
	selectIndex = uintptr(t.DataUint32()) >> 2
